...
public class ThresherSharkEntity extends PathfinderMob implements GeoEntity {
	...
	public static final EntityDataAccessor<String> ANIMATION = SynchedEntityData.defineId(ThresherSharkEntity.class, EntityDataSerializers.STRING);
	public static final EntityDataAccessor<String> TEXTURE = SynchedEntityData.defineId(ThresherSharkEntity.class, EntityDataSerializers.STRING);
	public static final EntityDataAccessor<Boolean> DATA_Sprinting = SynchedEntityData.defineId(ThresherSharkEntity.class, EntityDataSerializers.BOOLEAN);
	...
	public ThresherSharkEntity(PlayMessages.SpawnEntity packet, Level world) {
		this(BenssharksModEntities.THRESHER_SHARK.get(), world);
	}

	public ThresherSharkEntity(EntityType<ThresherSharkEntity> type, Level world) {
		super(type, world);
		xpReward = 2;
		setNoAi(false);
		setMaxUpStep(0.6f);
		...
		this.moveControl = new MoveControl(this) {
			@Override
			public void tick() {
				if (ThresherSharkEntity.this.isInWater())
					ThresherSharkEntity.this.setDeltaMovement(ThresherSharkEntity.this.getDeltaMovement().add(0, 0.005, 0));
				if (this.operation == MoveControl.Operation.MOVE_TO && !ThresherSharkEntity.this.getNavigation().isDone()) {
					double dx = this.wantedX - ThresherSharkEntity.this.getX();
					double dy = this.wantedY - ThresherSharkEntity.this.getY();
					double dz = this.wantedZ - ThresherSharkEntity.this.getZ();
					float f = (float) (Mth.atan2(dz, dx) * (double) (180 / Math.PI)) - 90;
					float f1 = (float) (this.speedModifier * ThresherSharkEntity.this.getAttribute(Attributes.MOVEMENT_SPEED).getValue());
					ThresherSharkEntity.this.setYRot(this.rotlerp(ThresherSharkEntity.this.getYRot(), f, 10));
					ThresherSharkEntity.this.yBodyRot = ThresherSharkEntity.this.getYRot();
					ThresherSharkEntity.this.yHeadRot = ThresherSharkEntity.this.getYRot();
					if (ThresherSharkEntity.this.isInWater()) {
						ThresherSharkEntity.this.setSpeed((float) ThresherSharkEntity.this.getAttribute(Attributes.MOVEMENT_SPEED).getValue());
						float f2 = -(float) (Mth.atan2(dy, (float) Math.sqrt(dx * dx + dz * dz)) * (180 / Math.PI));
						f2 = Mth.clamp(Mth.wrapDegrees(f2), -85, 85);
						ThresherSharkEntity.this.setXRot(this.rotlerp(ThresherSharkEntity.this.getXRot(), f2, 5));
						float f3 = Mth.cos(ThresherSharkEntity.this.getXRot() * (float) (Math.PI / 180.0));
						ThresherSharkEntity.this.setZza(f3 * f1);
						ThresherSharkEntity.this.setYya((float) (f1 * dy));
					} else {
						ThresherSharkEntity.this.setSpeed(f1 * 0.05F);
					}
				} else {
					ThresherSharkEntity.this.setSpeed(0);
					ThresherSharkEntity.this.setYya(0);
					ThresherSharkEntity.this.setZza(0);
				}
			}
		};
	}
	...
	protected void defineSynchedData() {
		...
		this.entityData.define(TEXTURE, "thresher_shark");
	...
	protected void registerGoals() {
		...
		this.targetSelector.addGoal(8, new NearestAttackableTargetGoal(this, LivingEntity.class, true, true) {
			...
			public boolean canUse() {
				...
				double y = ThresherSharkEntity.this.getY();
				double z = ThresherSharkEntity.this.getZ();
				Entity entity = ThresherSharkEntity.this;
				Level world = ThresherSharkEntity.this.level();
	...
	public InteractionResult mobInteract(Player sourceentity, InteractionHand hand) {
		...
		return ThresherSharkRightClickedOnEntityProcedure.execute(world, x, y, z, entity, sourceentity);
	}
	...
	protected void tickDeath() {
		...
		if (this.deathTime == 20) {
			this.remove(ThresherSharkEntity.RemovalReason.KILLED);
			this.dropExperience();
		}
	}
