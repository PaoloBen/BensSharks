...
import net.mcreator.sharks.procedures.TigerSharkEntityVisualScaleProcedure;
...
public class TigerSharkRenderer extends GeoEntityRenderer<TigerSharkEntity> {
	...
	@Override
	public void preRender(PoseStack poseStack, TigerSharkEntity entity, BakedGeoModel model, MultiBufferSource bufferSource, VertexConsumer buffer, boolean isReRender, float partialTick, int packedLight, int packedOverlay, float red, float green,
			float blue, float alpha) {
		Level world = entity.level();
		double x = entity.getX();
		double y = entity.getY();
		double z = entity.getZ();
		float scale = (float) TigerSharkEntityVisualScaleProcedure.execute(entity);
